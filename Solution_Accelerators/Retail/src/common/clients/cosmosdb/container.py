...
class CosmosDBContainer:
    ...
    @classmethod
    async def create(cls, database_name: str, container_name: str, partition_key_name: str, client: CosmosClient, unique_keys: Optional[List[Dict[str, Any]]] = None):
        self = cls(database_name, container_name, partition_key_name, client, unique_keys)
        await self._initialize()
        return self
    
    async def _initialize(self):
        database = await self.client.create_database_if_not_exists(id=self.database_name)
        partition_key = PartitionKey(path="/" + self.partition_key_name)
        unique_key_policy = {'uniqueKeys': self.unique_keys} if self.unique_keys is not None else None
        self.container = await database.create_container_if_not_exists(id=self.container_name, partition_key=partition_key, unique_key_policy=unique_key_policy)
        
    async def create_item(self, id: str, partition_key: str, item: Dict[str, Any]) -> Union[Dict[str, Any], Exception]:
        try:
            item["id"] = id
    ...
    async def upsert_item(self, id: str, partition_key: str, item: Dict[str, Any]) -> Dict[str, Any]:
        try:
            item["id"] = id
            item[self.partition_key_name] = partition_key
    ...
    async def query_items(self, query: str, params: List[Dict[str, object]], partition_key: str) -> List[Dict[str, Any]]:
        try:
            results = self.container.query_items(
                query=query, parameters=params, partition_key=partition_key
            )
