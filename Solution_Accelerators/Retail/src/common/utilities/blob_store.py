...
from common.contracts.common.user_prompt import UserPromptPayload
...
class BlobStoreHelper:
    def __init__(self, logger: AppLogger, storage_account_name:str, container_name = None):
        ...
        if container_name:
            ...
            if self.blob_client:
                ...
                raise BlobStoreHelperNotInitializedError("Class BlobStoreHelper not initialized with connection string")


    async def upload_image_async(self, item: UserPromptPayload) -> HttpUrl:
        ...
        file_extension = "jpg"
        if len(parts) > 1:
            # extract file extension from value 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAA...'
            image_data = parts[1]
            file_extension = parts[0].split(";")[0].split("/")[1]
        ...
        self.logger.info(f"Uploaded image to blob store and generated sas url", properties=additional_properties)
        return HttpUrl(sas_url)

    async def generate_blob_sas_url(self, blob_name: str, expiry: Optional[datetime] = None) -> str:
    ...
    async def download_image_data(self, sasUrl: str) -> tuple:
        ...
        try:
            ...
            self.logger.info(f"Downloaded image from blob store", properties=additional_properties)
