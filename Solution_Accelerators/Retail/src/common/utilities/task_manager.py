...
class TaskManager:
    ...
    """
    def __init__(self, logger: AppLogger, queue_name: str, redis_host: str, redis_port: int, redis_password: str, redis_ssl: bool, max_worker_threads: Optional[int] = None):
        self.logger = logger
    ...
    async def start_async(self, on_task_payload_received):
        ...
        await asyncio.gather(*(self.__task_worker(on_task_payload_received) for _ in range(self.max_worker_threads))) if self.max_worker_threads else await self.__task_worker(on_task_payload_received)
    ...
    async def __task_worker(self, on_task_payload_received):
        ...
        try:
            while self._processing_event.is_set():
                ...
                if task_payload:
                    self.logger.info(f"Task Manager: found task payload for processing.")
