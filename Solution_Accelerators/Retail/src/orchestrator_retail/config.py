    ...
    def initialize(cls):
        if not cls._initialized:
            ...
            try:
                ...
                cls.AZURE_OPENAI_GPT_SERVICE_FINAL_ANSWER_GENERATOR = config_reader.read_config_value(
                    Config.AZURE_OPENAI_ENDPOINT
                )

                cls.SEARCH_SKILL_URI = config_reader.read_config_value(Config.SEARCH_SKILL_URI)
                cls.IMAGE_DESCRIBER_SKILL_URI = config_reader.read_config_value(Config.IMAGE_DESCRIBER_SKILL_URI)
                cls.RECOMMENDER_SKILL_URI = config_reader.read_config_value(Config.RECOMMENDER_SKILL_URI)
                ...
                cls.REDIS_TASK_QUEUE_CHANNEL = config_reader.read_config_value(
                    Config.MULTIMODAL_BOT_REDIS_TASK_QUEUE_CHANNEL
                )
                cls.REDIS_MESSAGE_QUEUE_CHANNEL = config_reader.read_config_value(
