...
def load_file(filepath: str, filetype: str) -> dict:
    try:
        ...
        raise FileProcessingError(f"Failed to parse the local file content. Error: {str(e)}")
...
def download_file_from_sas_url(sas_url: str) -> str:
    try:
        ...
        raise FileDownloadError(f"Failed to download the file from SAS URL: {sas_url}. Error: {str(e)}")
...
def parse_file_content(file_content: str, file_extension: str) -> dict:
    try:
        ...
        raise FileProcessingError(f"Unexpected error during file parsing. Error: {str(e)}")
