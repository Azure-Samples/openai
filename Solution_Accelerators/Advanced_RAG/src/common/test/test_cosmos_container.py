...
from clients.cosmosdb.container import CosmosDBContainer
...
def upsert_item(body: Dict[str, Any]) -> Dict[str, Any]:
    return copy.deepcopy(body)

def upsert_item_raise_exception(body: Dict[str, Any]) -> Dict[str, Any]:
    raise Exception("Test exception.")
...
class CosmosDBContainerTests(unittest.TestCase):
    ...
    def test_upsert_item(self):
        # set up container with mocked dependencies
        cosmos_container_proxy = Mock()
        cosmos_container_proxy.upsert_item.side_effect = upsert_item
        cosmos_database_proxy = Mock()
        cosmos_database_proxy.create_container_if_not_exists.return_value = cosmos_container_proxy
        cosmos_client_mock = Mock()
        cosmos_client_mock.create_database_if_not_exists.return_value = cosmos_database_proxy
        ...
        # run test
        item = {"property1": "value1", "property2": "value2"}
        replacement_item = container.upsert_item("id", "partition_key", item)

        # assert container proxy was called as expected and returned replacement item as expected 
        cosmos_container_proxy.upsert_item.assert_called_once_with(item)
        self.assertDictEqual(item, replacement_item)

    def test_upsert_item_raises_exception(self):
        # set up container with mocked dependencies
        cosmos_container_proxy = Mock()
        cosmos_container_proxy.upsert_item.side_effect = upsert_item_raise_exception
        cosmos_database_proxy = Mock()
        cosmos_database_proxy.create_container_if_not_exists.return_value = cosmos_container_proxy
        cosmos_client_mock = Mock()
        cosmos_client_mock.create_database_if_not_exists.return_value = cosmos_database_proxy
        ...
        with self.assertRaises(Exception):
            item = {"property1": "value1", "property2": "value2"}
            container.upsert_item("id", "partition_key", item)
