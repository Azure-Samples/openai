'''
After modifying this file, also run it to autogen the json schemas templates
'''
...
def generate_json_config_schema_file(output_path: str):
    output = json.dumps(SearchSkillConfig.model_json_schema(), indent=2)
    print(output)
    with open(output_path, "w") as outfile:
        outfile.write(output)

def get_valid_config(config_path: str) -> Tuple[bool, SearchSkillConfig]:
    try:
        ...
        config = SearchSkillConfig(**config_content)
        return True, config
