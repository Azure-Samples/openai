...
class ResolvedOrchestratorConfig(BaseModel):
    ...
    service_configs: Optional[List[ServiceConfig]] = Field(default_factory=list)

    @staticmethod
    def _get_agent_name(config: AgentConfig) -> Optional[str]:
        """Extract agent name from different config types."""
        if isinstance(config.config_body, AIFoundryAgentConfig):
            # AIFoundryAgentConfig uses 'name' field
            return config.config_body.name
        else:
            # Other agent configs use 'agent_name' field
            return config.config_body.agent_name
    ...
    def validate_agent_configs(cls, agent_configs: Dict[str, AgentConfig]) -> Dict[str, AgentConfig]:
        ...
        for config in agent_configs.values():
            agent_name = cls._get_agent_name(config)
            if agent_name:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        agent_configs_dict = {}
        for _, config in self.agent_configs.items():
            agent_name = self._get_agent_name(config)
            if agent_name:
                agent_configs_dict[agent_name] = config.model_dump()

        result = {
            "base_config": self.base_config.model_dump(),
            "agent_configs": agent_configs_dict,
            "service_configs": (
                [config.model_dump() for config in self.service_configs] if self.service_configs else []
            ),
        }
    ...
    def validate(self):
        ...
        if self.agent_configs:
            ...
            valid_service_ids = (
                {service.config_body.service_id for service in self.service_configs} if self.service_configs else set()
            )
    ...
    def get_agent_config(self, agent_name: str) -> Optional[AgentConfig]:
        ...
        """
        agent_config = self.agent_configs.get(agent_name) or self.agent_configs.get(agent_name.lower())
        if agent_config:
