...
class SearchQueryGeneratorAgent(AzureAIAgentBase):
    """
    SearchQueryGeneratorAgent helps generate search queries at different levels for the Market Research Analyst.
    ...
    def __init__(self, logger: AppLogger, memory: Optional[SemanticTextMemory] = None):
        """Initialize the SearchQueryGeneratorAgent instance."""
    ...
    async def _create_azure_ai_agent(
        ...
        """
        Create the Search Query Generator agent in AI Foundry.
        ...
        if not configuration.azure_ai_agent_config:
            raise ValueError("Azure AI agent configuration is required for Search Query Generator Agent.")
