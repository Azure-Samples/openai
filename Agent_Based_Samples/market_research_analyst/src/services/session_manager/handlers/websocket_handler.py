...
from common.exceptions import ClientConnectionClosedException, ContentSafetyException
    ...
    async def process_message_queue_async(self):
        ...
        while True:
            try:
                ...
                self.logger.error(f"Content safety check failed for connection {self.session_id}: {cse}")

                message_json = json.loads(message)
                error_message = Response(
                    session_id=self.session_id,
                    dialog_id=message_json.get("dialog_id"),
                    user_id=message_json.get("user_id"),
                    answer=Answer(),
                    error=Error(
                        error_str=str(cse.message),
                        retry=True,
                        status_code=400,
                    )
                )
