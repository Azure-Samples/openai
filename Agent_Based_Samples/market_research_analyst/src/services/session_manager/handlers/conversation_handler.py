...
class ConversationHandler:
    ...
    async def validate_user_payload(self, request: Request) -> bool:
        """
        ...
        """
        text_moderator = TextModerator(DefaultConfig.AZURE_CONTENT_SAFETY_SERVICE, self.logger)
        image_moderator = ImageModerator(DefaultConfig.AZURE_CONTENT_SAFETY_SERVICE, self.logger)
        ...
        try:
            ...
            for item in request.message.payload:
                if item.type == PayloadType.IMAGE and DefaultConfig.IMAGE_CONTENT_SAFETY_CHECK_ENABLED:
                    # Check image safety
                    image_task = asyncio.create_task(image_moderator.is_safe_async(item.value))
                    image_safety_tasks.append(image_task)
                elif item.type == PayloadType.TEXT and DefaultConfig.TEXT_CONTENT_SAFETY_CHECK_ENABLED:
                    # Check text safety
                    text_safety_task = asyncio.create_task(text_moderator.is_safe_async(item.value))
                    text_safety_tasks.append(text_safety_task)

            await asyncio.gather(*image_safety_tasks)
            await asyncio.gather(*text_safety_tasks)

            return True
        except (UnsafeTextException, UnsafeImageException, ImageSizeException, Exception) as e:
            self.logger.exception(f"Content Safety check failed: {e}")
            raise ContentSafetyException("Your message was flagged for containing potentially unsafe content. Please rephrase your request.")

    async def upload_image_to_storage_async(self, request: Request) -> List[str]:
        """
        Uploads images from the request payload to Azure Blob Storage asynchronously.

        Args:
            request (Request): The request containing the image payloads.

        Returns:
            List[str]: A list of URIs for the successfully uploaded images.
        """
        ...
        try:
            ...
            self.logger.exception(f"Exception occurred while uploading images to storage: {e}")
            raise
    ...
    async def handle_request_message_async(self, message: str) -> None:
        ...
        # Content safety check for user request payload.
        await self.validate_user_payload(user_request)
    ...
    async def __create_user_message(self, request: Request) -> UserMessage:
        ...
        """
        image_uris = await self.upload_image_to_storage_async(request) if request.message.payload else []
