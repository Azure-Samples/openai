    ...
    def initialize(cls):
        if not cls._initialized:
            ...
            try:
                ...
                cls.AGENT_ORCHESTRATOR_MAX_CONCURRENCY = int(config_reader.read_config_value(Config.AGENT_ORCHESTRATOR_MAX_CONCURRENCY))

                cls.AZURE_AI_BING_GROUNDING_CONNECTION_NAME = config_reader.read_config_value(Config.AZURE_AI_BING_GROUNDING_CONNECTION_NAME)

                cls.AZURE_AI_SEARCH_ENDPOINT = config_reader.read_config_value(Config.AZURE_AI_SEARCH_ENDPOINT)
                cls.AZURE_AI_SEARCH_INDEX_NAME = config_reader.read_config_value(Config.AZURE_AI_SEARCH_INDEX_NAME)
                cls.AZURE_OPENAI_ENDPOINT = config_reader.read_config_value(Config.AZURE_OPENAI_ENDPOINT)
                cls.AZURE_OPENAI_EMBEDDINGS_DEPLOYMENT_NAME = config_reader.read_config_value(
                    Config.AZURE_OPENAI_EMBEDDING_DEPLOYMENT_NAME
                )
                cls.AZURE_OPENAI_API_VERSION = config_reader.read_config_value(Config.AZURE_OPENAI_API_VERSION)

                cls.SERVICE_HOST = os.getenv(Config.SERVICE_HOST.value, "0.0.0.0")
                cls.SERVICE_PORT = int(os.getenv(Config.SERVICE_PORT.value, "5003"))
