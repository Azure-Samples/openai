...
blob_client = BlobServiceClient.from_connection_string(DefaultConfig.AZURE_BLOB_CONNECTION_STRING)
...
def chat():
    ...
    if not chat_session_exists:
        ...
        logger.info(f"created new chat session for user {user_id} and session {conversation_id}", extra=properties)
    else:
        chat_session = data_client.get_chat_session(user_id, conversation_id)
        logger.info(f"chat session for user {user_id} and session {conversation_id} already exists", extra=properties)
    ...
    try:
        ...
        if approach_type == ApproachType.chit_chat:
            ...
            data_client.add_dialog_to_chat_session(user_id, conversation_id, ParticipantType.assistant, datetime.datetime.now(), json.dumps(response.answer.to_item()), DialogClassification.chit_chat)
            logger.info(f'added response {chit_chat_canned_response} to chat session for user {user_id} and session {conversation_id}', extra=properties)
        ...
        if access_manager.is_user_allowed(allowed_approaches, e.suggested_classification):
            response = ChatResponse(answer=Answer(f"Error when querying knowledge-base: '{str(e.message)}'."), show_retry=True, suggested_classification=e.suggested_classification, classification=question_classification)
            return jsonify(response.to_item())
        else:
            response = ChatResponse(answer=Answer(str(e.message)), classification=question_classification)
            return jsonify(response.to_item())
...
def clear_chat_session(user_id: str, conversation_id: str):
    ...
    try:
        ...
        logger.exception(f"Exception in /chat-sessions/<user_id>/<conversation_id>: {e}")
        return jsonify({"error": str(e)}), 500
